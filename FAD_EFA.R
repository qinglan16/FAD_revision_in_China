...
df3 <- read.table("FAD_3.csv",header = TRUE, sep = ",")    #n=142 (collected from OSF),exluded 8 cases, time < 60 sec
#recode df1
...
gendertable1 <- with(df1, table(gender_self_report,education_level))
...
t.test(df1$FD ~ df1$gender_self_report) #m=1.34,f=1.57,[-0.26, -0.18],p < 2.2e-16
t.test(df1$SD ~ df1$gender_self_report) #m=2.17,f=2.18,[-0.04,0.02],p = 0.5394
t.test(df1$UP ~ df1$gender_self_report) #m=2.18,f=2.19,[-0.04,0.02], p = 0.5588
t.test(df1$FW ~ df1$gender_self_report) #m=2.20,f=2.24,[-0.07,-0.01],p = 0.01804
...
psych::fa(df1.fadcor, nfactors = 4, n.obs = 27, rotate = "oblimin", fm = "ml")   #method: oblimin rotation and maximun likelihood extraction


#EFA for df2
df2.fad <- subset(df2, select = c(FD1:FW21,SD22:UP27))
df2.fadcor <- cor(df2.fad,use = "complete.obs")   #listwise for missing value
psych::fa(df2.fadcor, nfactors = 4, n.obs = 27, rotate = "oblimin", fm = "ml")   #method: oblimin rotation and maximun likelihood extraction


#EFA for df3
df3.fad <- subset(df3, select = c(FD1:UP27))
df3.fadcor <- cor(df3.fad,use = "complete.obs")   #listwise for missing value
psych::fa(df3.fadcor, nfactors = 4, n.obs = 27, rotate = "oblimin", fm = "ml")   #method: oblimin rotation and maximun likelihood extraction
